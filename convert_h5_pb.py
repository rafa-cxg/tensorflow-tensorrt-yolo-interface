...
#from yolo3.model import preprocess_true_boxes, yolo_body, tiny_yolo_body, yolo_head, yolo_loss
...
def load_model(input_model_path, input_json_path=None, input_yaml_path=None):
    ...
    try:
   #    model = keras.models.load_model(input_model_path, custom_objects={'yolo_head': yolo_head, 'yolo_loss': yolo_loss})
  ...
  #      model.load_weights(input_model_path)
        ...
        if input_json_path:
            ...
            if not Path(input_yaml_path).exists():
                raise FileNotFoundError(
                    'Model description yaml file `{}` does not exist.'.format(input_yaml_path))
            try:
