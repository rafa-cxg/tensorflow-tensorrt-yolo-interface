...
_WARMUP_NUM_LOOPS = 1
_LOG_FILE = "log.txt"
_LABELS_FILE = "config424.json"
_GRAPH_FILE = "fire424.pb"
...
def get_trt_graph_from_calib_cxg(graph_name, graph_def, data, input_node, output_node, output_dir):
    """Convert a TensorRT graph used for calibration to an inference graph."""
...
def time_graph(graph_def, data, input_node, output_node, num_loops=1):
...
def main(argv):
    ...
    flags.output_node = ['conv_81/BiasAdd', 'conv_93/BiasAdd', 'conv_105/BiasAdd']
    flags.input_node = "input_1"
    flags.int8 = 'int8'
    flags.frozen_graph = "fire424.pb"
    flags.batch_size= 8
    flags.output_dir = './'
    flags.image_file='/home/cxg/k-yolov3-gcp/fire-data/train/images/img (31).jpg'
    # Load the data.
    if flags.image_file:
        data_directory = "/home/cxg/k-yolov3-gcp/fire-data/calib"
        ...
        for i in  range(len(calibration_files)):
            ...
            batch_images.append(batch_from_image(calibration_files[i-1], 4))



        data = tf.data.TFRecordDataset( (batch_images))
    ...
    if flags.int8:
        ...
        int8_graph = get_trt_graph_from_calib_cxg(g_name, frozen_graph_def, data, flags.input_node, flags.output_node,
                                                  flags.output_dir)
